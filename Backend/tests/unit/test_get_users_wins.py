...
def test_get_users_wins_returns_empty_on_failed_users_fetch(monkeypatch):
    ...
    monkeypatch.setattr(App, "determine_user_roster_numbers", lambda: None)
    monkeypatch.setattr(App, "create_user_dictionary", lambda _users: pytest.fail("unexpected user creation"))
...
def test_get_users_wins_regular_season_path(monkeypatch):
    ...
    monkeypatch.setattr(App, "get_total_weeks", lambda _year: 18)
    monkeypatch.setattr(App, "determine_user_roster_numbers", lambda: None)
    monkeypatch.setattr(App, "set_winners_and_losers", lambda: pytest.fail("playoff seeding should not run"))
    monkeypatch.setattr(App, "drop_week_extremes_from_brackets", lambda week: pytest.fail("playoff drop should not run"))

    call_log = {}

    def fake_create_user_dictionary(_payload):
        App.users_dict.clear()
        App.users_dict["u1"] = User("Alpha", "u1", "Team A", App.LEAGUE_ID)
        App.users_dict["u2"] = User("Beta", "u2", "Team B", App.LEAGUE_ID)

    def fake_set_season_rankings(target_dict, min_week, max_week):
...
def test_get_users_wins_runs_playoff_path(monkeypatch):
    ...
    def fake_create_user_dictionary(_payload):
        App.users_dict.clear()
        App.users_dict["u1"] = User("WinnerOne", "u1", "Team A", App.LEAGUE_ID)
        App.users_dict["u2"] = User("WinnerTwo", "u2", "Team B", App.LEAGUE_ID)
        App.users_dict["u3"] = User("Loser", "u3", "Team C", App.LEAGUE_ID)
    ...
    def fake_set_winners_and_losers():
        nonlocal winners_called
        winners_called = True
        App.users_dict["u1"].bracket = "winners"
        App.users_dict["u2"].bracket = "winners"
        App.users_dict["u3"].bracket = "losers"

    def fake_calculate_weekly_points(target_dict, week):
        calc_weeks.append(week)
        for user in target_dict.values():
            user.points_per_week[week] = 50.0 if user.user_id == "u3" else 100.0

    def fake_drop_week_extremes_from_brackets(week):
        drop_weeks.append(week)
        if week == 13:
            loser = App.users_dict["u3"]
